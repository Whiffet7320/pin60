		...
		<view class="items">
			<view class="item" v-for="(item,i) in list" :key="item.id">
				...
				<view class="nav2">
					<view class="box1">
						<view class="box1-1">
							<view class="tit1">收货人：{{item.realname}}</view>
							<view class="tit2">联系方式：{{item.mobile}}</view>
						</view>
						<view class="box1-2">收货地址：{{item.city}}{{item.address}}</view>
					</view>
...
<script>
	export default {
		...
		methods: {
			...
			toXiugaixinxi(item) {
				uni.navigateTo({
					url: `./xinjian?realname=${item.realname}&mobile=${item.mobile}&city=${item.city}&address=${item.address}&is_default=${item.is_default}&id=${item.id}`
				})
			},
